...
// Create a Supabase admin client to bypass RLS securely
        ...
        systemPrompt += `\n\nPENTING: Untuk mode ini, anda MESTI memulangkan response dalam format JSON yang sah SAHAJA. Jangan masukkan sebarang teks sebelum atau selepas JSON blok. Format JSON mesti mengikut spesifikasi berikut:
{
  ...
  "notes": [
    "Nota tambahan"
      ...
      // Assuming model is something like 'deepseek-chat' mapped from the frontend choice
      ...
      const response = await fetch("https://api.deepseek.com/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          temperature: finalTemperature, // mapped from frontend
  ...
  app.get("/api/github/callback", async (req, res) => {
    ...
    try {
      ...
      // UPSERT connection info
      // In a real app we would want a secure way, but we will store access token in plain text
      // to meet the MVP requirement
